 ...
 *
 * Abbildungen: Wikimedia-Commons-Tafeln (keine Mathsachs-Eigen-Schemata).
 */
...
/** Jmarchn — frontal, unlabeled (CC BY-SA 3.0). */
const FEMALE_REPRO_SRC = '/anatomy/female-reproductive-frontal.png'
const FEMALE_REPRO_ATTR =
  'Jmarchn: „Scheme female reproductive non-labels-2“ (frontaler Querschnitt, unbeschriftet). Wikimedia Commons, CC BY-SA 3.0 — https://creativecommons.org/licenses/by-sa/3.0/ · https://commons.wikimedia.org/wiki/File:Scheme_female_reproductive_non-labels-2.svg (ShareAlike: Bearbeitungen unter gleicher/kompatibler Lizenz; Datei unverändert, Drop-Felder nur UI-Overlay)'

/** T. Kebert — frontal, unlabeled (CC BY-SA 4.0). */
const MALE_REPRO_SRC = '/anatomy/male-reproductive-frontal.png'
const MALE_REPRO_ATTR =
  'T. Kebert: „Male reproductive frontal without labels“. Wikimedia Commons, CC BY-SA 4.0 — https://creativecommons.org/licenses/by-sa/4.0/ · https://commons.wikimedia.org/wiki/File:Male_reproductive_frontal_without_labels.svg (ShareAlike: Bearbeitungen unter gleicher/kompatibler Lizenz; Datei unverändert, Drop-Felder nur UI-Overlay)'

/** Topic-appropriate distractors only (no fish/bird anatomy). */
const FEMALE_DISTRACTORS = ['Hoden', 'Samenleiter', 'Vorsteherdrüse'] as const
const MALE_DISTRACTORS = ['Eierstock', 'Eileiter', 'Gebärmutter'] as const
...
function femaleLabel(rng: Rng) {
  const distractors = shuffle(rng, [...FEMALE_DISTRACTORS]).slice(0, 2)
  ...
  return imageLabelSlotsTask({
    ...
    imageAlt: 'Weibliche Geschlechtsorgane im frontalen Querschnitt (unbeschriftet)',
    ...
    instruction: 'Ziehe die Bezeichnungen in die Felder. Die Linien zeigen auf die Organe.',
    fachwissen: bioFw(
    ...
    contentIds: ['female-repro:diagram', ...FEMALE_SLOTS.map((s) => s.concept)],
...
function maleLabel(rng: Rng) {
  const distractors = shuffle(rng, [...MALE_DISTRACTORS]).slice(0, 2)
  ...
  return imageLabelSlotsTask({
    ...
    imageAlt: 'Männliche Geschlechtsorgane von vorn (unbeschriftet)',
    ...
    contentIds: ['male-repro:diagram', ...MALE_SLOTS.map((s) => s.concept)],
...
function cycleCloze(rng: Rng) {
  const variants = [
    {
      template: 'In den ___ werden Eizellen gebildet; die ___ bereitet sich auf eine mögliche Schwangerschaft vor.',
      accepted: [
